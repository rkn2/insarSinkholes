...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--quality-score-col",
        default=None,
        help="Optional column used to gate alerts. If set, raw_pred requires quality>=quality-min-score.",
    )
    p.add_argument("--quality-min-score", type=float, default=0.0)
    return p.parse_args()
...
def _apply_policy(
    ...
    cooldown_days: int,
    quality_score_col: str | None = None,
    ...
    if quality_score_col and quality_score_col in out.columns:
        q = pd.to_numeric(out[quality_score_col], errors="coerce").fillna(0.0)
        out["raw_pred"] = ((out["raw_pred"] == 1) & (q >= float(quality_min_score))).astype(int)
...
def _event_cv_score(df: pd.DataFrame, policy: dict) -> dict:
    ...
    for event_id, g in df.groupby("event_id"):
        applied = _apply_policy(
            ...
            quality_score_col=policy.get("quality_score_col"),
            quality_min_score=float(policy.get("quality_min_score", 0.0)),
        )
        m = _metrics(applied)
        m["event_id"] = event_id
        by_event.append(m)
            ...
            for persist in persistence_values:
                for cooldown in cooldown_values:
                    policy = {
                        ...
                        "quality_score_col": args.quality_score_col,
